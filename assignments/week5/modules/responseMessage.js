module.exports = {
    NULL_VALUE: "필요한 값이 없습니다.",
    OUT_OF_VALUE: "파라미터 값이 잘못 되었습니다.",
  
    /* Member */
    MEMBER_CREATE_SUCCESS: "회원 생성 성공",
    MEMBER_READ_SUCCESS: "회원 조회 성공",
    MEMBER_READ_ALL_SUCCESS: "전체 회원 조회성공",
    MEMBER_UPDATE_SUCCESS: "회원 수정 성공",
    MEMBER_DELETE_SUCCESS: "회원 삭제 성공",
  
    /* 회원가입 */
    SIGN_UP_SUCCESS: "회원가입성공",
    SIGN_UP_FAIL: "회원 가입 실패",
    SIGN_IN_SUCCESS: "로그인 성공",
    SIGN_IN_FAIL: "로그인 실패",
    ALREADY_ID: "존재하는 ID 입니다.",
    ALREADY_EMAIL: "존재하는 EMAIL 입니다.",
    NO_USER: "존재하지않는 유저 id 입니다.",
    MISS_MATCH_PW: "비밀번호가일치하지않습니다",

    /** User */
    USER_READ_ALL_FAIL: "모든 유저 읽기 실패.",
    USER_READ_ALL_SUCCESS: "모든 유저 읽기 성공.",
    USER_UPDATE_SUCCESS: "유저 업데이트 성공.",
    USER_UPDATE_FAIL: "유저 업데이트 실패.",
    USER_DELETE_SUCCESS: "유저 삭제 성공.",
    USER_DELETE_FAIL: "유저 삭제 실패.",

    /** Post */
    CREATE_POST_SUCCESS: "포스트 생성 성공.",
    CREATE_POST_FAIL: "포스트 생성 실패.",
    READ_ALL_POST_SUCCESS: "모든 포스트 읽기 성공.",
    READ_ALL_POST_FAIL: "모든 포스트 읽기 실패.",
    CREATE_LIKE_SUCCESS: "좋아요 성공",
    CREATE_LIKE_FAIL: "좋아요 실패.",

    /* 서버에러 */
    INTERNAL_SERVER_ERROR: "서버 내부 오류",
}